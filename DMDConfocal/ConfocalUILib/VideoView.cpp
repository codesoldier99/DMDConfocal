...
BEGIN_MESSAGE_MAP(CVideoView, CWnd)
	...
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_COMMAND(ID_VIDEOVIEW_FITWINDOW, &CVideoView::OnEpiFitwindows)
	ON_COMMAND(ID_VIDEOVIEW_SAVEFRAME, &CVideoView::OnEpiSaveframe)
	ON_WM_MOUSEWHEEL()
  ON_COMMAND(ID_VIDEOVIEW_ALARM, &CVideoView::OnVideoviewAlarm)
END_MESSAGE_MAP()

CVideoView::CVideoView(void)
{
	//初始化调色板
...
BOOL CVideoView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
...
bool CVideoView::InitPanel (LPVOID p_Param, LPVOID p_Param2)
{
...
void CVideoView::Renderer( HVideoHeader* pHeader,LPBYTE pBuffer )
{
...
void CVideoView::Render(LPBYTE pBuffer, int width,int heidth,int wbit )
{
  ...
  if (m_Buffer && m_alarm)
  {
    if (wbit == 24)
    {
      int w = m_bmi->bmiHeader.biWidth;
      int h = m_bmi->bmiHeader.biHeight;
      for (int i = 0; i < h; i++)
      {
        for (int j = 0; j < w; j++)
        {
          int p = (i * w + j) * 3;
          if (m_Buffer[p] == 255 && m_Buffer[p + 1] == 255 && m_Buffer[p + 2] == 255)
          {
            // 设置成红色
            m_Buffer[p] = 0;
            m_Buffer[p + 1] = 0;
...
void CVideoView::DoubleDCFun( CDC* pDC,CRect pRc )
{
	//准备工作
	CDC MemDC;
...
void CVideoView::DrawFunctionM2(CDC* pDC,CRect pRc)
{
...
void CVideoView::DrawFunction(CDC* pDC,CRect pRc)
{
	ZoomAmp=1.0f;
	pDC->FillSolidRect(0,0,pRc.Width(),pRc.Height(),afxGlobalData.clrBarFace);
  EnterCriticalSection(&m_cs);
...
void CVideoView::DrawRuler( CDC* pDC,CPoint p_x1,CPoint p_x2 )
{
	if(p_x1==p_x2)
		return;
...
void CVideoView::DrawFps(CDC* pDC,CRect pRc,float pFps)
{
	Graphics g(pDC->m_hDC);
	CString text;
...
void CVideoView::update_scrollbar( int width,int height )
{
	if(m_bFitWndows)
	{
		width=0;
...
void CVideoView::UpdataView(int pHpos,int pVpos )
{
	if(pHpos<0)
		pHpos=0;
	if (pVpos<0)
...
void CVideoView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	SCROLLINFO info;
	memset( &info, 0, sizeof( info ) );
...
void CVideoView::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	CMenu tmenu;
	tmenu.LoadMenuW(IDR_ViedoViewMENU);//加入菜单
...
void CVideoView::OnEpiFitwindows()
{
	// TODO: 在此添加命令处理程序代码
...
void CVideoView::OnEpiSaveframe()
{
	// TODO: 在此添加命令处理程序代码
	CString ImgName;
	SYSTEMTIME st;
...
int CVideoView::OnSubjectNotified(IPtnSubject * pSubject,int ID,long wParam,void* pParam,float fParam,void* mParam)
{
...
void CVideoView::EnableAlarm(bool enable)
{
  m_alarm = enable;
  if (!m_bmi)
  {
  ...
  if (m_alarm)
  {
    m_bmi->bmiColors[255].rgbBlue = 0;
    m_bmi->bmiColors[255].rgbGreen = 0;
    m_bmi->bmiColors[255].rgbRed = 255;
  }
  else
  {
    m_bmi->bmiColors[255].rgbBlue = 255;
    m_bmi->bmiColors[255].rgbGreen = 255;
    m_bmi->bmiColors[255].rgbRed = 255;
  }
}

void CVideoView::OnVideoviewAlarm()
{
  m_alarm = !m_alarm;
  EnableAlarm(m_alarm);
}
