...
bool SIView::InitPanel( LPVOID p_Param, LPVOID p_Param2)
{
...
void SIView::UpdataView(int pHpos,int pVpos )
{
	if(pHpos<0)
		pHpos=0;
	if (pVpos<0)
...
void SIView::Render(LPBYTE pBuffer, int width,int heidth,int wbit )
{
...
void SIView::Renderer( HVideoHeader* pHeader,LPBYTE pBuffer )
{
...
void SIView::update_scrollbar( int width,int height )
{
	if(m_bFitWndows)
	{
		width=0;
...
void SIView::DrawFunction(CDC* pDC,CRect pRc)
{
...
void SIView::DrawFunctionM2(CDC* pDC,CRect pRc)
{
	pDC->FillSolidRect(0,0,pRc.Width(),pRc.Height(),afxGlobalData.clrBarFace);
	if(m_Buffer)// pvar_wndBitmap->dibits != NULL )
	{
...
void SIView::DoubleDCFun( CDC* pDC,CRect pRc )
{
	//一大段的开始
	CDC MemDC;
	CBitmap MemBitmap;
...
void SIView::DrawRuler( CDC* pDC,CPoint p_x1,CPoint p_x2 )
{
	if(p_x1==p_x2)
		return;
...
void SIView::DrawFps(CDC* pDC,CRect pRc,float pFps)
{
	Graphics g(pDC->m_hDC);
...
BEGIN_MESSAGE_MAP(SIView, CWnd)
	...
	ON_WM_MOUSEMOVE()
	ON_WM_TIMER()
	ON_COMMAND(ID_MODE_START_SI, &SIView::OnModeStartSI)
	ON_COMMAND(ID_MODE_SAVEFRAME, &SIView::OnModeSaveframe)
	ON_COMMAND(ID_MODE_STOP_SI, &SIView::OnModeStopSI)
	ON_COMMAND(ID_MODE_SAVEPROCESSIMG, &SIView::OnModeSaveprocessimg)
	ON_COMMAND(ID_MODE_FITWINDOW, &SIView::OnFITWINDOW)
END_MESSAGE_MAP()


BOOL SIView::OnEraseBkgnd(CDC* pDC)
{
...
void SIView::OnSize(UINT nType, int cx, int cy)
{
...
void SIView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	SCROLLINFO info;
...
void SIView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	SCROLLINFO info;
	memset( &info, 0, sizeof( info ) );
...
void SIView::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	m_bDragMouse=true;
	m_MouseX=point;
...
void SIView::OnRButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	//if(!m_bUpdata)
	//	SetTimer(529,30,NULL);// 刷新进程
...
void SIView::OnRButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	ClientToScreen(&point);
	CMenu tmenu;
	tmenu.LoadMenuW(IDR_MENU_SI);//加入菜单
...
void SIView::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	m_bDragMouse=false;
	m_MouxeY=point;
...
void SIView::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if(m_bDragMouse)
		m_MouxeY=point;
...
void SIView::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
void SIView::OnModeStartSI()
{
	// TODO: 在此添加命令处理程序代码
	this->Notify(this,NOTIFY_SI_START);
}

void SIView::OnModeStopSI()
{
	// TODO: 在此添加命令处理程序代码
	this->Notify(this,NOTIFY_SI_STOP);
}

void SIView::OnModeSaveframe()
{
	// TODO: 在此添加命令处理程序代码
	this->Notify(this,NOTIFY_SI_SAVERESULT);
}

void SIView::OnModeSaveprocessimg()
{
	// TODO: 在此添加命令处理程序代码
	this->Notify(this,NOTIFY_SI_SAVEPROCESS);
}

void SIView::OnFITWINDOW()
{
