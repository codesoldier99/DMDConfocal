...
CRenderWnd::CRenderWnd(void)
{
	...
	m_bmi->bmiHeader.biBitCount = 16;//原来没有，设置16位

	m_bmi->bmiHeader.biSize=sizeof(BITMAPINFOHEADER);
	for(int i=0;i<255;i++)//调色板   255
	{
	...
	lf.lfHeight = 13 ;  
	brush=new SolidBrush(Color(255, 255, 0, 0));  
...
BOOL CRenderWnd::OnEraseBkgnd(CDC* pDC)
{
	...
	if (m_Buffer)
	{
		...
		DrawFps(&MemDC,rc,m_FpsLastValue);
...
void CRenderWnd::DrawFps(CDC* pDC,CRect pRc,float pFps)
{
...
{
	DWORD newSize=width*heidth*wbit/wbit;//8
	...
	if(newSize!=m_bmi->bmiHeader.biSizeImage)
	{
		...
		m_bmi->bmiHeader.biSizeImage=width*heidth*wbit/wbit;//8
		if(m_Buffer)
		{
			delete(m_Buffer);
			m_Buffer=0;
...
void CRenderWnd::DrawROIS( CDC* pDC)
{
	if(m_Buffer)
	{
		...
		g.DrawRectangle(&ROIPen,mX,mY,
			m_DrawROISize.x,m_DrawROISize.y);
		...
		CString text;
		text.Format(_T("(%d,%d)\r\nSize(%d,%d)"),mROIRealRec.left,mROIRealRec.top,mROIRealRec.Width(),mROIRealRec.Height());
		g.DrawString(text.GetBuffer(),text.GetLength(),font,PointF(10,10),&stringformat,brush);
	}
}
