...
bool HSaveBmp( LPBYTE pBuffer,int IWidth, int IHeight, int wBitsPerPixel, LPCTSTR lpszFileName )
{
	...
	bmpinfoHeader.biSizeImage=IWidth*IHeight*(wBitsPerPixel/ wBitsPerPixel);//8
...
void ChangeFormate( unsigned char* scr,unsigned char* dest )
{
	...
	for (int ih=0;ih<768;ih++)
	{
		for (int iw=0;iw<128;iw++)
		{
			...
			{
				char vK=0x0;
				if (scr[1024*ih + iw*8 + ik]>125)
					vK=0x1;
				else
					vK=0x0;
				//dest[128*ih + iw]=(dest[128*ih + iw] << 1)|(scr[1024*ih + iw*8 + ik] & 1);
				dest[128*ih + iw]=(dest[128*ih + iw] << 1)|vK;
			}
		}
	}
}
